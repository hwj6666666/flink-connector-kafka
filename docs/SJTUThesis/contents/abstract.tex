...
\begin{abstract}[zh]
  ...
  该原型能够在不重启 Flink 作业的前提下发现符合规则的 Topic、建立对应写入路径，并完成多目标写入与 route 级状态记录。
