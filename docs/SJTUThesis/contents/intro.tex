...
\begin{figure}[htbp]
  \centering
  \begin{tikzpicture}[
    node distance=0.42cm and 1.0cm,
    box/.style={draw, rounded corners, align=center, minimum width=2.5cm, minimum height=0.62cm, fill=gray!8},
    arrow/.style={-Latex, thick},
    title/.style={font=\small\bfseries, align=center}
  ]
    \node[title] (oldtitle) {传统停机更新};
    \node[box, below=of oldtitle] (oldjob) {Flink 作业};
    \node[box, below=of oldjob] (stop) {停机};
    \node[box, below=of stop] (modify) {修改配置};
    \node[box, below=of modify] (fixed) {固定 Topic};

    \node[title, right=3.0cm of oldtitle] (newtitle) {动态发现与路由};
    \node[box, below=of newtitle] (run) {Flink 作业\\持续运行};
    \node[box, below=of run] (match) {模式匹配};
    \node[box, below=of match] (route) {路由映射};
    \node[box, below=of route] (topic) {新增 Topic\\自动纳入};

    \draw[arrow] (oldjob) -- (stop);
    \draw[arrow] (stop) -- (modify);
    \draw[arrow] (modify) -- (fixed);
    \draw[arrow] (run) -- (match);
    \draw[arrow] (match) -- (route);
    \draw[arrow] (route) -- (topic);
    \draw[arrow] (fixed.east) -- (topic.west);
    \node[align=center, font=\small, below=0.15cm of topic, xshift=-2.2cm] {写入目标变化不再直接触发作业重启};
  \end{tikzpicture}
...
还必须处理 producer 配置、目标 Topic 绑定、写入通道生命周期管理、状态快照与提交语义等一系列更复杂的问题。
...
底层写入通道的懒创建与复用、Checkpoint 状态快照、路由级提交与恢复等多个环节的协同设计。
...
如何在流处理连接器中把元数据刷新、生产者生命周期、路由绑定、状态恢复与提交语义组织成完整机制，
...
并支持至少一次与精准一次等投递语义，但常规 Kafka Sink 仍要求在构建阶段绑定目标 Topic。
这意味着 Flink 运行时已经具备支撑动态行为的状态与提交框架，
但官方静态 Sink 使用方式尚未充分覆盖动态 Topic 发现、路由级写入通道管理和运行期目标切换。
...
这一场景，也不需要在同一个 Flink Sink 写入执行模块内部处理多物理 Topic 的懒创建、状态封装和路由级提交。
...
目标 Topic 绑定、写入 fan-out、底层写入通道生命周期、事务提交与恢复状态组织，两者的状态语义和失败边界并不相同。
...
对“如何在一个可运行原型中组织正则 Topic 发现、逻辑路由到物理 Topic 的映射、多写入通道复用、
Checkpoint 状态快照以及 exactly-once 事务路径”的完整实现与实验验证仍较有限。
...
使作业能够在运行期感知目标变化、维护逻辑路由与物理 Topic 的映射、
按路由管理底层写入通道与状态，并尽可能复用既有 KafkaSink 的写入与提交能力。
...
\begin{enumerate}
  ...
  \item 基于现有代码实现元数据发现、运行时路由、写入器管理、状态恢复与提交协调等连接器核心能力，完成动态路由管理、状态恢复与提交链路；
  \item 以实验作业作为载体，对 connector 暴露出的动态发现、显式路由更新与静态基线能力进行验证，
