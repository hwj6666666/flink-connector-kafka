...
\subsection{显式路由与静态基线实验配置}

\begin{codeblock}[caption={用于显式路由与静态基线实验的配置样例}]
dynamic:
...
发现实时性实验没有依赖显式 route map，而是使用自动发现实验入口生成不携带路由标识的数据记录，
并通过 \id{stream_pattern} 驱动元数据刷新与路由集合更新。对应命令中使用的关键参数为：
