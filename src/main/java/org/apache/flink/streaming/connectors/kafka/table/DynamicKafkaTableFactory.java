...
/** Factory for creating configured instances of dynamic Kafka table source and sink. */
@Internal
public class DynamicKafkaTableFactory
    ...
    public DynamicTableSink createDynamicTableSink(Context context) {
        ...
        final DeliveryGuarantee deliveryGuarantee = tableOptions.get(DELIVERY_GUARANTEE);
        final KafkaPartitioner<RowData> partitioner =
                getFlinkKafkaPartitioner(tableOptions, context.getClassLoader()).orElse(null);

        return new DynamicKafkaTableSink(
                physicalDataType,
                physicalDataType,
                ...
                false,
                parallelism,
                tableOptions.getOptional(TRANSACTIONAL_ID_PREFIX).orElse(null),
    ...
    private static void validateTableSinkOptions(ReadableConfig tableOptions) {
        validateStreams(tableOptions);
        validateMetadataService(tableOptions);
        if (tableOptions.getOptional(STREAM_IDS).isPresent()
                && tableOptions.get(STREAM_IDS).size() != 1) {
            throw new ValidationException(
                    "Dynamic Kafka sink currently supports exactly one configured stream-id.");
        }
    }
